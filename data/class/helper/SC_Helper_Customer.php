...
class SC_Helper_Customer {
        ...
        if (count($arrRet) > 0) {
            ...
            if ($arrRet[0]['del_flg'] != '1') {
                ...
                if ($pass_time < $limit_time) {
                    // 再登録制限期間内削除ユーザー
                    //return 2; // ======= Changed by KACR 2013.12.19 ========
                }
            }
        }
    ...
    function sfCustomerEntryParam(&$objFormParam, $isAdmin = false) {
        ...
        if ($isAdmin) {
        	...
        	$objFormParam->addParam('お支払い方法', 'payment_id', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK'));
        	// ===================================================
        }
    ...
    function sfCustomerRegisterParam(&$objFormParam, $isAdmin = false, $is_mypage = false) {
        ...
        $objFormParam->addParam('性別', 'sex', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'), '1');
        ...
        $objFormParam->addParam('メールマガジン', 'mailmaga_flg', INT_LEN, 'n', array('NUM_CHECK', 'MAX_LENGTH_CHECK'), '3');
