...
class SC_DB_DBFactory_MYSQL extends SC_DB_DBFactory {
    ...
    function getOrderTotalDaysWhereSql($type) {
        ...
        return " date_format(create_date, '".$format."') AS str_date,
            ...
            SUM(CASE WHEN payment_id = 6 THEN 1 ELSE 0 END) AS men,
            SUM(CASE WHEN payment_id = 3 THEN 1 ELSE 0 END) AS women,
            SUM(CASE WHEN payment_id = 7 THEN 1 ELSE 0 END) AS conveni,
            SUM(CASE WHEN cst = 2 THEN 1 ELSE 0 END) AS men_member,
            SUM(CASE WHEN payment_id = 3 AND status <> 6 THEN 1 ELSE 0 END) AS women_member,
            SUM(CASE WHEN cst = 1 THEN 1 ELSE 0 END) AS men_nonmember,
            SUM(CASE WHEN payment_id = 3 AND status = 6 THEN 1 ELSE 0 END) AS women_nonmember,
            SUM(total) AS total,
            SUM(case WHEN status = 6 then total else 0 end) AS total_average";
    }
