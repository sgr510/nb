...
class LC_Page_Entry extends LC_Page_Ex {
    ...
    function action() {
        ...
        // ================ Added by KACR 2013.12.11 =================
        // 配送業者を取得
        $arrSelectedDeliv = $this->getSelectedDeliv();
        $this->arrPayment = $arrSelectedDeliv['arrPayment'];
        $this->total_cost = $arrSelectedDeliv['total'];
        if (!empty($_POST['payment_id']))
        {
        	foreach ($this->arrPayment as $val){
        		if ($val['payment_id'] == $_POST['payment_id']){
        			$this->payment_method = $val['payment_method'];
        		}
        	}
        }
        if (isset($_SESSION['entry_sess']) && !empty($_SESSION['entry_sess'])){
        	$this->arrForm  = $_SESSION['entry_sess'];
        }
        // ===========================================================
        ...
        switch ($this->getMode()) {
            case 'confirm':
                ...
                if (empty($this->arrErr)) {
                    ...
                    // =============== Added by KACR 2013.12.12 ===============
                    $this->doCart($arrSelectedDeliv['product_class_id'], $this->arrForm);
                    // ========================================================
            ...
            case 'complete':
                ...
                if (empty($this->arrErr)) {
        			...
        			$sqlval = $this->lfMakeSqlVal($objFormParam);
        			if ($this->use_module) {
        				$sqlval['status'] = 2;
        			}else{
        				$sqlval['status'] = 1;
        			}
        			if (isset($sqlval['payment_id'])){unset($sqlval['payment_id']);}
        			
        			$uniqid             = $this->lfRegistCustomerData($sqlval);
        			$objQuery =& SC_Query_Ex::getSingletonInstance();
        			$customer_id = $objQuery->get("customer_id", "dtb_customer", "secret_key = ?", array($uniqid));
        			
	                
	                // 集計結果を受注一時テーブルに反映
	                $this->tpl_uniqid = $objSiteSess->getUniqId();
	                $this->saveOrderTemp($this->tpl_uniqid, $this->arrForm, $objPurchase);
	                ...
	                if ($this->use_module) 
	                {
	                	$_SESSION['entry_sess'] = $this->arrForm;
	                	
	                	// paypal決済設定
	                    $_SESSION['customer_uniqid'] = $uniqid;
	                    
	                    $order_id = $objPurchase->completeOrder_custom(ORDER_PENDING, $customer_id);
	                    $_SESSION['order_id'] = $order_id;
	                    
	                    // 会員情報の削除フラグを設定
	                    $objQuery->update('dtb_customer', array('del_flg'=>1), 'customer_id = ?', array($customer_id));
	                ...
	                $objPurchase->completeOrder_custom(ORDER_NEW, $customer_id);
    ...
    function lfSendMail($uniqid, $arrForm) {
        ...
        //if (CUSTOMER_CONFIRM_MAIL == true) {
            ...
            $toCustomerMail = $objMailText->fetch('mail_templates/customer_half_mail.tpl');
        //} else {
     ...
     * 支払い方法を取得する.
     *
     * @author KACR 2013.12.11
     * @return array 支払い方法を格納した配列
     */
    function getSelectedDeliv() 
    {
    	$product_type_id = 1; //通常商品
        $objPurchase = new SC_Helper_Purchase_Ex();
        ...
        $arrResults = array();
        $arrResults['product_class_id'] = 0;
        
        $objProduct = new SC_Product_Ex();
        $objQuery =& SC_Query_Ex::getSingletonInstance();
        $customer_product_flg = 1;
        $select = "A.price02, A.product_class_id ";
        $where = "A.del_flg = 0 AND B.del_flg = 0 AND A.product_type_id = ? AND B.customer_product_flg = ? ";
        $from = "dtb_products_class A LEFT JOIN dtb_products B ON A.product_id = B.product_id";
        $arrProduct = $objQuery->getRow($select, $from, $where, array($product_type_id, $customer_product_flg));
        $arrProduct = (array)$arrProduct;

		if (!empty($arrProduct)){
			$total = $arrProduct['price02'];
			$arrResults['product_class_id'] = $arrProduct['product_class_id'];
		}
 		
		$total = SC_Helper_DB_Ex::sfCalcIncTax($total);
       
        $arrResults['arrPayment'] = $objPurchase->getPaymentsByPrice($total, $deliv_id);
        $arrResults['total'] = $total;
        
        
        return $arrResults;
    }
    
	function doCart($product_class_id, $arrForm) 
	{
		$objCartSess = new SC_CartSession_Ex();
		$objCartSess->delAllProducts(1);
		$quantity = 1;
		...
		$this->tpl_uniqid = $objSiteSess->getUniqId();
		$this->registerDeliv($shipping_id, $this->tpl_uniqid, $objPurchase, $arrForm);
		
		$objPurchase->setShipmentItemTempForSole($objCartSess, $shipping_id);
		$objSiteSess->setRegistFlag();
    }
    
	function registerDeliv($other_deliv_id, $uniqid, &$objPurchase, $arrForm) 
	{
        $arrValues = array();
        ...
        foreach ($keys as $key) {
        	if (in_array($key, $keys)) {
        		$arrValues[$prefix . '_' . $key] = $arrForm[$key];
        	}
        }
        ...
        //$this->saveOrderTemp($uniqid, $arrValues, $objPurchase);
    }
    
	function saveOrderTemp($uniqId, $params, &$objPurchase, $prefix = 'order') 
	{
        ...
        $cols = array('name01', 'name02', 'kana01', 'kana02',
            'sex', 'zip01', 'zip02', 'pref', 'addr01', 'addr02',
            'tel01', 'tel02', 'tel03', 'fax01', 'fax02', 'fax03',
            'job', 'birth', 'email'
        );
        ...
        foreach ($params as $key => $val) {
            if (in_array($key, $cols)) {
                $sqlval[$prefix . '_' . $key] = $val;
            }
        }
        
		$sqlval['device_type_id'] = SC_Display_Ex::detectDevice();
        $sqlval['session'] = serialize($_SESSION);
        
        $arrDeliv = $objPurchase->getDeliv(1); //$product_type_id
        $sqlval['deliv_id'] = $arrDeliv[0]['deliv_id'];
        
        $objCartSess = new SC_CartSession_Ex();
        $arrRet = $objCartSess->getCartList(1);

		$price = $arrRet[0]['price'];
		$incTax = SC_Helper_DB_Ex::sfTax($price);
		$total = SC_Helper_DB_Ex::sfCalcIncTax($price);
		$sqlval['subtotal'] 	 = $total;
		$sqlval['tax'] 			 = $incTax;
		$sqlval['total']		 = $total;
		$sqlval['payment_total'] = $total;
		$sqlval['payment_id'] 	 = $params['payment_id'];
		foreach ($this->arrPayment as $val){
			if ($val['payment_id'] == $params['payment_id']){
				$sqlval['payment_method'] = $val['payment_method'];
			}
		}
        
        $exists = $objPurchase->getOrderTemp($uniqId);
