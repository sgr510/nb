...
__EOS__;
    ...
    function lfGetOrderTerm($type, $sdate, $edate) {
        ...
        list($where, $arrWhereVal) = $this->lfGetWhereMember('o.create_date', $sdate, $edate);
        $where .= ' AND o.del_flg = 0 AND c.del_flg=0 AND o.status <> ?';
        $arrWhereVal[] = ORDER_CANCEL;
        ...
        // 検索結果の取得
        $arrTotalResults = $objQuery->select($col, 'dtb_order o left join (select customer_id cid, status cst, del_flg from dtb_customer) c on (o.customer_id=c.cid) ', $where, $arrWhereVal);
