...
class LC_Page_Mdl_Paypal_Helper_Link extends LC_Page_Ex {
    ...
    function init() {
        ...
        $this->tpl_title = 'クレジットカードまたはPayPalでお支払い';
        
        // ========= Changed by KACR 2013.12.20 ==========
        $this->httpCacheControl('nocache'); //private
        // ===============================================
    }
    ...
    function action() {
        ...
        // =============== Added by KACR 2013.12.13 ================
    	if (isset($_SESSION['customer_uniqid'])){
			$arrOrder['return'] = ENTRY_COMPLETE_URL;
	...
	// ======== Added by KACR 2013.12.23 =========        
        if (!isset($_SESSION['customer_uniqid'])){
        	$objFormParam->addParam("notify_url", "notify_url", URL_LEN, "KVa", array("MAX_LENGTH_CHECK"), PAYPAL_NOTIFY_URL);
        }else{
        	$objFormParam->addParam("notify_url", "notify_url", URL_LEN, "KVa", array("MAX_LENGTH_CHECK"), PAYPAL_NOTIFY_URL."?entry_flg=true");
        }
	// ===========================================
