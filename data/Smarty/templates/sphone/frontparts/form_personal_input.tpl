...
<!--{if $flgFields > 3}-->
        ...
        <dd>
        	<p>年間登録料　¥<!--{$total_cost|number_format}-->円（税込）</p>
            <p>年間登録料のお支払いはクレジットカード、Paypal、または銀行振込がご利用いただけます。</p>
            <!--{assign var=key1 value="`$prefix`payment_id"}-->
	        <!--{if $arrErr[$key1]}-->
	            <div class="attention"><!--{$arrErr[$key1]}--></div>
	        <!--{/if}-->
	        <ul id="payment" style="<!--{$arrErr[$key1]|sfGetErrorColor}-->">
	        	<!--{section name=cnt loop=$arrPayment}-->
                <li>
                	<input type="radio" id="pay_<!--{$smarty.section.cnt.iteration}-->" name="<!--{$key1}-->" value="<!--{$arrPayment[cnt].payment_id}-->" style="<!--{$arrErr[$key1]|sfGetErrorColor}-->" <!--{$arrPayment[cnt].payment_id|sfGetChecked:$arrForm[$key1].value}--> class="data-role-none" />
