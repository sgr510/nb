    ...
    <table class="list" id="customer-search-result">
        ...
        <!--{foreach from=$arrData item=row}-->
            ...
            <tr>
                ...
                <td><!--{mailto address=$row.email encode="javascript"}--></a><!--{if $row.status eq 1}--><br /><a href="#" onclick="return fnReSendMail('<!--{$row.customer_id|h}-->');">本登録メール送信</a><!--{/if}--></td>
            </tr>
        <!--{/foreach}-->
    </table>
    <!--検索結果表示テーブル-->
