...
<!--{* 2013.12.05 Added by KACR *}-->        
        <tr>
            <th>会員用コンテンツ<span class="attention">(タグ許可)*</span></th>
            <td>
                <span class="attention"><!--{$arrErr.customer_comment}--></span>
                <textarea name="customer_comment" maxlength="<!--{$smarty.const.LLTEXT_LEN}-->" style="<!--{if $arrErr.customer_comment != ""}-->background-color: <!--{$smarty.const.ERR_COLOR}-->;<!--{/if}-->" cols="60" rows="8" class="area60"><!--{"\n"}--><!--{$arrForm.customer_comment|h}--></textarea><br />
                <span class="attention"> (上限<!--{$smarty.const.LLTEXT_LEN}-->文字)</span>
            </td>
        </tr>
        <tr>
            <th>nofollowフラグ</th>
            <td>
                <input type="checkbox" name="nofollow_flg" value="1" <!--{if $arrForm.nofollow_flg == "1"}-->checked<!--{/if}--> />nofollowにする
            </td>
        </tr>
<!--{* ======================== *}-->        
        <tr>
